...
resource "aws_iam_role" "devops_ec2_iam_role" {
...
resource "aws_iam_role_policy" "devops_ec2_iam_role_policy" {
  count = "${var.existing_ec2_iam_instance_profile_name == "" ? 1 : 0}"
  name = "${var.ec2_iam_role_name}-policy-${random_id.clusterid.hex}"
  role = "${aws_iam_role.devops_ec2_iam_role.id}"
  lifecycle  {
      create_before_destroy = true
    }
  policy = <<EOF
...
resource "aws_iam_role_policy_attachment" "devops_iam_s3fullaccess" {
  count = "${var.existing_ec2_iam_instance_profile_name == "" ? 1 : 0}"
  role = "${aws_iam_role.devops_ec2_iam_role.name}"
  policy_arn = "arn:aws:iam::aws:policy/AmazonS3FullAccess"
}

resource "aws_iam_instance_profile" "devops_ec2_instance_profile" {
  count = "${var.existing_ec2_iam_instance_profile_name == "" ? 1 : 0}"
